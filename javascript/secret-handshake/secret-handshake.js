...
export const commands = (num) => {
  const actions = [];
  const binary = num.toString(2).padStart(5, '0'); // Ensure binary representation is 5 digits long

  // Define actions and their corresponding binary positions
  const actionMap = {
    1: "wink",
    2: "double blink",
    3: "close your eyes",
  ...
  // Iterate through binary digits to determine actions
  for (let i = binary.length - 1; i >= 0; i--) {
    const digit = binary[i];
    if (digit === '1') {
      const action = actionMap[binary.length - i];
      if (action) actions.push(action);
    }
  }

  // Reverse actions if fifth binary digit is 1
  if (binary[0] === '1') {
    actions.reverse();
  }
